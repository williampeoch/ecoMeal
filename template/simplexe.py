...
# méthode du simplexe pour réoudre
...
def simplexe(A,b,c,permut):
	...
	# "m:" le range "m,m+1,...,n-1" et ":" le range "0,1,...,n-1"
        ...
        vmax=max(xrmax)
        # on met les valeurs négatives Ã  une valeur grande
        for i in range(m):
            if xrmax[i]<= 0:
                xrmax[i]=vmax+1
